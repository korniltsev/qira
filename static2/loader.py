...
def load_binary(static):
  try:
    ...
    logging.error("*** loader error: non-ELF detected")
    return
  ...
  for section in elf.iter_sections():
    if static.debug >= 2:
      logging.error("** found section %s %s", section.name, type(section))
    ...
    if isinstance(section, RelocationSection):
      ...
      for rel in section.iter_relocations():
        ...
        if static.debug >= 2: #suppress output for testing
          logging.error("Relocation %s %s",rel, symbol.name)
    ...
    if isinstance(section, SymbolTableSection):
      for nsym, symbol in enumerate(section.iter_symbols()):
        ...
        if symbol['st_value'] != 0 and symbol.name != "" and symbol['st_info']['type'] == "STT_FUNC":
          if static.debug >= 2:
            logging.error("Symbol %s %s",hex(symbol['st_value']), symbol.name)
  ...
  if static.debug >= 1:
    logging.error("** found %d names", ncount)
